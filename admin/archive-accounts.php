...
ON pt.user=us.username

WHERE 
username LIKE '%$searchValue%' AND pt.status='scheduled delete'
OR email LIKE '%$searchValue%' AND pt.status='scheduled delete'
OR first_name LIKE '%$searchValue%' AND pt.status='scheduled delete'
OR other_names LIKE '%$searchValue%' AND pt.status='scheduled delete'
OR mobile_number LIKE '%$searchValue%' AND pt.status='scheduled delete'
ORDER BY pt.id DESC";
...
WHERE pt.status='scheduled delete'
ORDER BY pt.id DESC";
$result= filterTable($query);
}                                      
...
<!--if there are users, display them-->
    ...
    <tr>
      ...
      <td><?php echo $row['delete_on']; ?></td>

 <!--the action buttons-->     
<td>
<a href="dboperations.php?undoDelete=<?php echo $row['id']; ?>" class="badge bg-primary fa fa-undo" onclick="return confirm('This account will be recovered and all previous operations reverted')">Undo</a></td>  
</td>
